...
let mount = function () {
    if (isMobile) {
        ...
        target += `phone=${encodeURIComponent(telefone)}&text=Assunto: ${encodeURIComponent(assunto.value)}\n Nome: ${encodeURIComponent(nome.value)}\n Mensagem: ${encodeURIComponent(mensagem.value)}\n`
...
$('.btnWpp').on('click', function () {
    ...
    if (!!mensagem && mensagem.value !== '' && !!assunto && assunto.value !== '' && !!nome && nome.value !== '') {
        mount();
    }
});